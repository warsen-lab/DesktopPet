...
- **按住宠物拖动 → 松手** → 自由落体掉下 → 生气 → 走向最近的桌面图标位置「吃掉」它（盖一块可还原的遮挡）。
- **右键宠物** 或 **托盘菜单「还原被吃掉的图标」** → 还原所有遮挡。
- **退出**：托盘图标右键 →「退出」（或终端 `Ctrl+C`）。

> 「吃图标」是**纯视觉覆盖**，不碰你的真实文件/图标，随时可还原。精确识别真实图标坐标需原生模块，当前用桌面图标网格近似。
...
- [x] 拖拽彩蛋：自由落体 → 生气 → 吃图标（可还原遮挡）+ 托盘
- [ ] 吃图标升级：原生模块识别真实桌面图标坐标，精确"咬"图标
- [ ] 更多动作集（stand/run/angry/eat 专用帧，见素材指南）
