...
形象支持两种模式，设置里**一键切换、即时生效**：
- **2D 照片精灵**：用你家宠物的真实照片/视频生成的精灵帧。
- **3D 模型**：three.js 渲染 .glb 宠物模型（自带骨骼时自动程序化行走，自带动画剪辑则直接播放）。
...
- **托盘菜单**：设置（渲染模式 2D/3D / 开机自启 / 休息 / 拉屎频率 / 跑步速度）、宠物素材工坊、清理大便、退出。托盘图标就是你的宠物。

> 大便是画在置顶层的视觉物，不碰你的真实文件，随时可清理。宠物位置钳制在所在屏的**工作区**内，不会被屏幕/任务栏边缘裁切。

## 素材与配置都在用户目录（升级不丢）

- 2D 素材：`%APPDATA%/desktop-pet/pets/cat/`，首次启动自动从内置素材复制，之后**升级/重装都不会覆盖**你的自定义素材。
- 3D 模型：`%APPDATA%/desktop-pet/pets/model/`，放入你自己的 `.glb` 即可替换（Tripo/Meshy 等生成的带骨骼模型最佳）。
...
渲染进程（每窗口一份，只画一块屏）
   ...
   ├─ renderers/Renderer2D.js  canvas 画真猫精灵（多动画集 + 程序化特效）
   ├─ renderers/Renderer3D.js  three.js 加载 .glb：有动画剪辑用 AnimationMixer，
   │                           无剪辑但有骨骼（Tripo 自动绑骨）→ 程序化步态（四肢对角摆+尾巴+呼吸）
   └─ vendor/                  three.module.js / GLTFLoader 等（vendored，无打包器直接 ESM 加载）
UI 窗口（src/ui/）
   ├─ settings.html  设置：渲染模式(2D/3D) / 开机自启 / 休息 / 拉屎频率 / 跑步速度 / 检查更新
   └─ guide.html     素材工坊：拍摄角度线条示意 + 素材状态 + 打开文件夹 / 重扫热重载
```

- **多屏**：Windows 透明窗口无法可靠跨多屏，故一屏一窗口；宠物全局坐标模拟、各窗口减偏移绘制；位置钳制在可视范围内不被边缘裁切；混合 DPI 也正确。
- **2D/3D 切换**：设置里改「渲染模式」→ 主进程重建覆盖窗口，`app.js` 按模式实例化渲染器；行为层与主进程模拟完全不动。3D 模型字节由主进程经 IPC 传给渲染进程解析（绕开 file:// 的 fetch 限制），3D 初始化失败自动回退 2D。
...
- [x] v0.3.0：3D 模型渲染（three.js + .glb）、设置一键切换 2D/3D、骨骼程序化行走、用户可替换自己的 .glb
